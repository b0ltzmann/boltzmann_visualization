...
class Boltz:
    ...
    def makeplot(self):
        ...
        # Text for q:
        self.qtext = self.ax.text(0.7, 0.6, '$q=$',
                                  size=15, transform=self.ax.transAxes)
        # Text for model:
        self.ax.text(0.7, 0.9, self.model,
                     size=15, transform=self.ax.transAxes)
        # Sliders locations:
        ...
        self.temp_slider     = Slider(ax=axtemp,
        ...
        if self.model == 'PiB':
            self.mass_slider = Slider(ax=axmass,
                                      label='Mass, $m$ / amu',
                                      valmin=1,
                                      valmax=100,
                                      valinit=self.m0)
            self.box_slider  = Slider(ax=axbox,
                                      label='Length, $L$ / nm',
                                      valmin=0.1,
                                      valmax=10,
                                      valinit=self.L0)
            self.ax.text(0.7, 0.8, r'$E_n=\frac{n^2h^2}{8mL^2}$',
                         size=15, transform=self.ax.transAxes)
            self.ax.text(0.7, 0.7, r'$g_n=1$ $\forall$ $n$',
                         size=15, transform=self.ax.transAxes)
        if self.model == 'QMRR':
            self.mass_slider = Slider(ax=axmass,
            ...
            self.box_slider  = Slider(ax=axbox,
                                      ...
                                      valmax=300,
                                      valinit=self.r0)
            self.ax.text(0.7, 0.8, r'$E_J=\frac{\hbar^2}{2\mu r^2}J(J+1)$',
                         size=15, transform=self.ax.transAxes)
            self.ax.text(0.7, 0.7, r'$g_J=2J+1$',
                         size=15, transform=self.ax.transAxes)
        if self.model == 'QMHO':
            self.mass_slider = Slider(ax=axmass,
                                      label='Reduced Mass, $\mu$ / amu',
            ...
            self.box_slider  = Slider(ax=axbox,
                                      ...
                                      valmax=3000,
                                      valinit=self.kf0)
            self.ax.text(0.7, 0.8, r'$E_v=(v+0.5)\hbar\sqrt{\frac{k_f}{\mu}}$',
                         size=15, transform=self.ax.transAxes)
            self.ax.text(0.7, 0.7, r'$g_v=1$ $\forall$ $v$',
                         size=15, transform=self.ax.transAxes)
    ...
    def addtoplot(self, energies, fractions, q, T):
        ...
        # Add kBT line
        self.ax.plot((0, 1), (self.kb*T, self.kb*T),
                     lw=2, ls=':', color='#cc3333')
    ...
    def update(self, val):
        ...
        """
        if self.model == 'PiB':
            energies = self.quantum(m=self.mass_slider.val,
                                    L=self.box_slider.val)
        if self.model == 'QMRR':
            energies = self.quantum(mu=self.mass_slider.val,
                                    r=self.box_slider.val)
        if self.model == 'QMHO':
            energies = self.quantum(mu=self.mass_slider.val,
                                    kf=self.box_slider.val)
    ...
    def initialize(self):
        ...
        """
        energies = self.quantum(m=self.m0,
                                L=self.L0,
                                kf=self.kf0,
                                mu=self.mu0,
                                r=self.r0)
